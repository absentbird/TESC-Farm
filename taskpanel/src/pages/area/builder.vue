<template>
  <div id="main-content" class="pl-8 pr-8 pb-8">
    <v-btn variant="tonal" @click="router.push('/area')" class="mb-8"
      >Go to Areas</v-btn
    >
    <v-form ref="form">
      <h2>Area Name:</h2>
...
const createArea = async () => {
  ...
  await apicall("/task/new", {
    area_id: r.ID,
    name: "General",
    description: "Work applicable to all crops in area",
  });
  await apicall("/task/new", {
    area_id: r.ID,
    name: "Lab",
    description: "Learning stuff in this area",
  });
  router.push("/area/" + r.ID);
};
</script>
